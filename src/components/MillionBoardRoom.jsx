...
import goldMemberBg from '../assets/gold_member_bg.jpg';

const MillionBoardRoom = () => {
  const navigate = useNavigate();
  const filter = new Filter();
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState('');
  const { address, isConnected } = useAccount(); // Replace address state with useAccount
  ...
  useEffect(() => {
    ...
    const setupListener = () => {
      try {
        const q = query(collection(db, "goldRoomChat"), orderBy("timestamp", "asc"));
          ...
          {isEditingUsername ? (
            ...
            >
              <input
                ...
                className="w-40 bg-transparent border-b border-white/30 px-1 py-0.5 text-white focus:outline-none focus:border-[#8B7355] text-sm placeholder-white/50"
                autoFocus
            ...
            >
              <span className="font-medium text-sm text-white group-hover:text-[#8B7355] transition-colors">
                {username}
              </span>
  ...
  const sendMessage = async (e) => {
    ...
    try {
      await addDoc(collection(db, "goldRoomChat"), {
        text: newMessage,
        sender: address,
        username: username,
    ...
    <div className="bg-[#1a1b1e] min-h-screen text-white pb-16">
      ...
      <div className="w-full h-[calc(100vh-4rem)] md:h-[calc(100vh-6rem)] max-w-[420px] md:max-w-[1200px] mx-auto md:my-4 flex flex-col bg-[#1E1E1E] rounded-none md:rounded-3xl overflow-hidden">
        ...
        <div 
          className="p-4"
          style={{ 
            backgroundImage: `url(${goldMemberBg})`,
        ...
        >
          ...
          <div className="text-center space-y-2">
            {usernameSection}
            <h1 className="text-xl font-semibold">Gold Members Lounge</h1>
          </div>
        </div>

        {/* Chat Buttons */}
        <div className="flex gap-2 p-2 bg-[#1E1E1E] max-w-[600px] mx-auto w-full">
          <button className="flex-1 bg-[#1a1b1e] text-white py-2 px-4 rounded-lg border border-[#8B7355] hover:bg-[#8B7355]/20 transition-colors text-sm">
            Gold Chat
          </button>
          <button 
            disabled
        ...
        <div className="p-3 bg-[#1E1E1E] border-t border-gray-800">
          <form onSubmit={sendMessage} className="flex gap-2 max-w-[800px] mx-auto">
            <input
              ...
              className="flex-1 rounded-lg bg-[#2A2A2A] px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#8B7355] text-white placeholder-gray-400"
            />
            <button 
              type="submit"
...
export default MillionBoardRoom; 